...
class DAT(nn.Module):
    ...
    def __init__(self, img_size=(512, 1024), patch_size=4, num_classes=1000, expansion=4,
                #  dim_stem=128, dims=[128, 256, 512, 1024], depths=[2, 2, 18, 2],  # DAT-Large pretrained model
                 dim_stem=256, dims=[256, 256, 256, 256], depths=[2, 2, 18, 2],  # Changed dimensions to match DPT-Large pretrained model
                 heads=[4, 8, 16, 32], 
        ...
        self.patch_proj2 = nn.Sequential(
            ...
            nn.Conv2d(3, dim_stem, patch_sizei//2, patch_size//2, 0),
            LayerNormProxy(dim_stem)
        ) 
        ...
        for i in range(4):
            ...
            dim1 = dim_stem if i == 0 else dims[i - 1]  # DPT-Large pretrained model
            dim2 = dims[i]
...
class Conv_Decoder(BaseModel):
    def __init__(
        self,
        # features=512,
        features=256,  # DPT-Large pretrained model
        readout="project",
        channels_last=False,
        use_bn=False,
        ...
        self.output_conv = nn.Sequential(
            ...
            nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1),  ## Pretrained DPT model
            Interpolate(scale_factor=2, mode="bilinear", align_corners=True),
            # nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(128, 32, kernel_size=3, stride=1, padding=1),  # Pretrained DPT model
            nn.ReLU(True),
