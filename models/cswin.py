...
class LePEAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, attn_drop=0., proj_drop=0., qk_scale=None):
        ...
        elif idx == 0:  # Horizontal Self-Attention
    ...
    def im2cswin(self, x):
        ...
        # print(f'x : {x.shape}, H_sp : {self.H_sp}, W_sp : {self.W_sp}, num_heads : {self.num_heads}, C : {C}')
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=[512, 1024], patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
        ...
        self.stage1 = nn.ModuleList([
            CSWinBlock(
                dim=curr_dim, num_heads=heads[0], reso=[img_size[0]//4, img_size[1]//4], mlp_ratio=mlp_ratio,
        ...
        temp_layer_rn.append(Tune_Block(curr_dim, 256, resolution = [img_size[0]//4, img_size[1]//4]))

        self.merge1 = Merge_Block(curr_dim, curr_dim*2, resolution = [img_size[0]//4, img_size[1]//4])
        curr_dim = curr_dim*2
        self.stage2 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[1], reso=[img_size[0]//8, img_size[1]//8], mlp_ratio=mlp_ratio,
        ...
        # Tuning into decoder dimension
        temp_layer_rn.append(Tune_Block(curr_dim, 256, resolution = [img_size[0]//8, img_size[1]//8]))

        self.merge2 = Merge_Block(curr_dim, curr_dim*2, resolution = [img_size[0]//8, img_size[1]//8])
        curr_dim = curr_dim*2
        temp_stage3 = []
        temp_stage3.extend(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[2], reso=[img_size[0]//16, img_size[1]//16], mlp_ratio=mlp_ratio,
        ...
        # Tuning into decoder dimension
        temp_layer_rn.append(Tune_Block(curr_dim, 256, resolution = [img_size[0]//16, img_size[1]//16]))
        
        self.merge3 = Merge_Block(curr_dim, curr_dim*2, resolution = [img_size[0]//16, img_size[1]//16])
        curr_dim = curr_dim*2
        self.stage4 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[3], reso=[img_size[0]//32, img_size[1]//32], mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[-1],
                drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        # Tuning into decoder dimension
        temp_layer_rn.append(Tune_Block(curr_dim, 256, resolution = [img_size[0]//32, img_size[1]//32]))
        self.layer_rn = nn.ModuleList(temp_layer_rn)
    ...
    def forward_features(self, x):
        ...
        for blk in self.stage1:
            if self.use_chk:
                ...
                x = blk(x)
        features.append(self.layer_rn[0](x))

        for idx, items in enumerate(zip([self.merge1, self.merge2, self.merge3], 
                                        [self.stage2, self.stage3, self.stage4])):
        # for pre, blocks in zip([self.merge1, self.merge2, self.merge3], 
        #                        [self.stage2, self.stage3, self.stage4]):
            pre, blocks = items[0], items[1]
            x = pre(x)
            for blk in blocks:
                if self.use_chk:
                    x = checkpoint.checkpoint(blk, x)
                    ...
                    x = blk(x)
            features.append(self.layer_rn[idx + 1](x))
        x = self.norm(x)
        return torch.mean(x, dim=1), features
